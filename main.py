...
def start(message):
    ...
    bot.send_message(message.chat.id, "እንኳን ደህና መጡ! ምን ላግዝዎት?", reply_markup=markup)
...
def handle_messages(message):
    if user_modes.get(message.chat.id) == 'AI':
        ...
        # (ቪዲዮ ዳውንሎደር እዚህ ይቀጥላል)
    else:
        bot.send_message(message.chat.id, "እባክዎ ከተዘረዘሩት አማራጮች አንዱን ይምረጡ።")

# --- 5. SAFE STARTUP (ይህ ነው መፍትሄው!) ---
if __name__ == "__main__":
    print("ቦቱ እየተነሳ ነው...")
    
    # የቆዩ ግንኙነቶችን እና የተከማቹ መልእክቶችን በሃይል ያጸዳል
    bot.remove_webhook()
    time.sleep(1)
    
    # ያለምንም ስህተት እንዲሰራ 'skip_pending' እዚህ ጋር አይጨመርም
